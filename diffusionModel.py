...
### MODIFICATION ###
# A. feature added: Added adaptive guidence strength so that we need not mention the delta by ourselves and it could optimize it according to the needs
#                   and also will converge faster than the previous method.
# B. new parameters: detail_min - the minima of guidance strength allowed
#                    detail_max - the maxima of guidance strength allowed
#                    similarity_threshold - the threshold parameter until which the parameter guidence strength will increase

class fgdModel(diffusionModel):
    def generateFGD(self, filter: fgdFilter, detail_min=0.5, detail_max=2, similarity_threshold=0.25, t_end=15, norm_steps=50, seed=10):
        filter.reset()
        seed_everything(seed)
        ...
        # initializing the delta with a mean value
        ...
        for t in tqdm(timesteps, leave=False):
            ...
            # calculating SSIM similarity in order to check for adaptive delta increase or decrease condition
